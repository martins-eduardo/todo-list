    ...
    <Card size="md">
      {!isEditing ? (
        <div className="flex items-center gap-4">
          <InputCheckbox checked={task.conclued} onChange={handleChangeTaskStatus} loading={loading} />
          {!loading ? (
            <Text
              className={cx("flex-1", {
        ...
        <form onSubmit={handleSaveTask} className="flex items-center gap-4">
          ...
          <div className="flex gap-1">
            <ButtonIcon icon={XIcon} type="button" variant="secondary" onClick={handleExitEditTask} />
            <ButtonIcon icon={CheckIcon} type="submit" variant="primary" handling={isUpdatingTask} />
          </div>
        </form>
      )}
    </Card>
